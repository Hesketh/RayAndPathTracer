...
BOOL AppWindow::InitWindow(HINSTANCE hInstance, int width, int height)
{
	m_hwnd = CreateWindowEx( WS_EX_APPWINDOW | WS_EX_WINDOWEDGE,
		L"AppWindow", L"TinyRay - Graphics II", WS_OVERLAPPED|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_SYSMENU,
	...
	//allocate the ray and path tracer and the scene
	m_pRayTracer = new RayTracer(width, height);
	m_pPathTracer = new PathTracer(width, height);

	//Set the ray tracer to be the currently active tracer
	m_pActiveTracer = m_pRayTracer;
...
void AppWindow::Render()
{
	...
	m_pActiveTracer->DoTrace(m_pScene);
	pBuffer = m_pActiveTracer->GetFramebuffer()->GetBuffer();
...
BOOL AppWindow::KeyUp(WPARAM key)
{
	...
	{
	case VK_F1:
		m_pRayTracer->m_traceflag = (RayTracer::TraceFlags)(RayTracer::TRACE_DIFFUSE_AND_SPEC |
			RayTracer::TRACE_SHADOW | RayTracer::TRACE_REFLECTION);
		m_pActiveTracer = m_pRayTracer;
		break;
	case VK_F2:
		m_pPathTracer->SetSampleCount(50);
		m_pActiveTracer = m_pPathTracer;
		break;
	case VK_F3:
		m_pPathTracer->SetSampleCount(100);
		m_pActiveTracer = m_pPathTracer;
		break;
	case VK_F4:
		m_pPathTracer->SetSampleCount(500);
		m_pActiveTracer = m_pPathTracer;
		break;
	default:
		break;
	...
	m_pActiveTracer->ResetRenderCount();
	return TRUE;
}