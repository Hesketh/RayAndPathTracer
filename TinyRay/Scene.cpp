...
RayHitResult Scene::IntersectByRay(Ray& ray)
{
	...
	//DONE: Implement ray to scene intersection
	...
	//Enumerate through each Primitive in the Scene
	for (Primitive* object : m_sceneObjects)
	{
		//Check if the ray instersects with the Primitive
		RayHitResult currentResult = object->IntersectByRay(ray);

		//Ensure we are not behind the origin of the ray
		if (currentResult.t >= 0)
		{
			//If this new ray is closer to the primitive then we will be returning it 
			result = (currentResult.t < result.t) ? currentResult : result;
		}
	}
